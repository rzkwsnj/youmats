<div>
    ...
    <h3 class="h-6 flex mb-8 text-sm font-bold">Orders Status <span class="ml-auto font-semibold text-gray-400 text-xs">({{ $totalOrders }} total)</span>
    </h3>
            ...
            <ul>
                @if($totalPendingOrder > 0)
                    <li class="text-xs leading-normal">
                        ...
                        pending <br /> ({{ $totalPendingOrder }} - {{ round(($totalPendingOrder / $totalOrders) * 100) }}%)
                    </li>
                @endif

                    @if($totalShippingOrder > 0)
                        <li class="text-xs leading-normal">
                            <span class="inline-block rounded-full w-2 h-2 mr-2" style="background-color: rgb(64, 153, 222);"></span>
                            shipping <br /> ({{ $totalShippingOrder }} - {{ round(($totalShippingOrder / $totalOrders) * 100) }}%)
                        </li>
                    @endif

                    @if($totalCompleteOrder > 0)
                        <li class="text-xs leading-normal">
                            <span class="inline-block rounded-full w-2 h-2 mr-2" style="background-color: rgb(64, 153, 222);"></span>
                            complete <br /> ({{ $totalCompleteOrder }} - {{ round(($totalCompleteOrder / $totalOrders) * 100) }}%)
                        </li>
                    @endif

                    @if($totalRefusedOrder > 0)
                        <li class="text-xs leading-normal">
                            <span class="inline-block rounded-full w-2 h-2 mr-2" style="background-color: rgb(64, 153, 222);"></span>
                            refused <br /> ({{ $totalRefusedOrder }} - {{ round(($totalRefusedOrder / $totalOrders) * 100) }}%)
                        </li>
                    @endif
            </ul>
        </div>
        <div class="w-1/2">
            @livewire(\App\Filament\Widgets\Embeded\OrderStatusChartWidget::class)
        </div>
    </div>
</div>
